...
router.get('/', function (req, res) {
  pool.getConnection(function (err, connection) {
    ...
    connection.query(query1, (err, results) => {
      ...
      let query2 = `
      SELECT DISTINCT A.Title as Title, A.ISBN as ISBN, A.rating as Rating, R.ReviewText as RatingText, R.RatingLikert as RatingLikert, B.Returned as Returned, B.BorrowDate as BorrowDate
      FROM (
      ...
      LEFT JOIN borrowing B ON A.ISBN = B.ISBN AND B.Idusers = ?;
      `;
      ...
      connection.query(query2, b, (err, results) => {
        ...
        for (const row of results) {
          ...
          if (row.Returned === 'False') {
            if (row.BorrowDate && (new Date() - new Date(row.BorrowDate)) > 14 * 24 * 60 * 60 * 1000) {
              statusText = 'Due';
            } else {
        ...
        htmlTable += `
            ...
            <button onclick="window.location.href='http://localhost:9103/libq/user'" class="back-button">Back to Home Page</button>
            </html>
        `;
